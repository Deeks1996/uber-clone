...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(rideDocRef, (snapshot) => {
      ...
      if (rideData) {
        setDriverName(rideData?.driverName || null);
  ...
  const handleModalClose = () => {
    ...
    router.push("/rider/dashboard").then(() => {
  ...
  return (
    <div className="min-h-screen bg-blue-200">
      ...
      {driverName && rideStatus === "accepted" && (
        <div className="text-center mt-6 text-xl text-slate-800 font-medium">
           Driver Assigned: <span className="font-bold">{driverName}</span>
        </div>
      )}
